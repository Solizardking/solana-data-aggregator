...
class DatasetMetricType(str, Enum):
    ...
    # Jupiter DEX swap quote metrics
    JUPITER_SOL_PRICE_USD = "jupiter_sol_price_usd"
    JUPITER_QUOTE_COUNT = "jupiter_quote_count"
    JUPITER_AVG_PRICE_IMPACT_BPS = "jupiter_avg_price_impact_bps"
    JUPITER_ROUTES_COUNT = "jupiter_routes_count"

    # Market signal metrics
    SIGNAL_AVG_CONFIDENCE = "signal_avg_confidence"
    SIGNAL_MARKETS_COUNT = "signal_markets_count"
...
_METRIC_METADATA: dict[DatasetMetricType, dict[str, str]] = {
    ...
    # Jupiter DEX
    DatasetMetricType.JUPITER_SOL_PRICE_USD: {
        "name": "Jupiter SOL Price (USD)",
        "unit": "USD",
        "description": "Average SOL/USD price derived from Jupiter SOL→USDC swap quotes in the Clawd dataset",
    ...
    DatasetMetricType.JUPITER_QUOTE_COUNT: {
        "name": "Jupiter Quote Count",
        "unit": "Count",
        "description": "Total Jupiter swap quotes captured in the Clawd training dataset",
    },
    DatasetMetricType.JUPITER_AVG_PRICE_IMPACT_BPS: {
        "name": "Jupiter Avg Price Impact (bps)",
        "unit": "bps",
        "description": "Average price impact in basis points across all Jupiter swap quotes in the Clawd dataset",
    },
    DatasetMetricType.JUPITER_ROUTES_COUNT: {
        "name": "Jupiter Unique Routes",
        "unit": "Count",
        "description": "Number of unique token-pair swap routes across all Jupiter quotes in the Clawd dataset",
    },
    # Market signals
    DatasetMetricType.SIGNAL_AVG_CONFIDENCE: {
        "name": "Signal Average Confidence",
        "unit": "Score",
        "description": "Average confidence score (0–1) from the Clawd signal discovery report across SOL/BTC/ETH markets",
        "methodology": "Mean confidence from signal_discovery_report.json generated by the Clawd autoresearch pipeline",
    },
    DatasetMetricType.SIGNAL_MARKETS_COUNT: {
        "name": "Signal Markets Count",
        "unit": "Count",
        "description": "Number of markets analyzed in the Clawd signal discovery report",
    },
}
